...
Richfaces.Calendar = {};
Richfaces.Calendar.setElementPosition = function (
  element,
  baseElement,
  jointPoint,
  direction,
  ...
  if (typeof jointPoint == "object") {
    ...
    jointPoint &&
    (match = jointPoint.toLowerCase().match(re)) != null
  ) {
  ...
  if (direction && (match = direction.toLowerCase().match(re)) != null) {
    ...
    if (match[2] == "left") ox -= elementDim.width + offset.dx;
    else if (match[2] == "right") ox += offset.dx;
    if (match[1] == "top") oy -= elementDim.height + offset.dy;
    else if (match[1] == "bottom") oy += offset.dy;
  } else {
    var theBest = { square: 0 };
    var basex = baseOffset[0] - offset.dx;
    var basey = baseOffset[1] + offset.dy;
    var rect = {
      right: basex + baseElementDim.width,
      top: basey + baseElementDim.height,
    };
...
Richfaces.Calendar.checkCollision = function (
  elementRect,
  windowRect,
  windowOffset
) {
  ...
  var rect = {
    left:
      elementRect.left > windowRect.left ? elementRect.left : windowRect.left,
...
Richfaces.Calendar.getWindowViewport = function () {
  ...
  return {
    ...
    right: windowDim.width + windowOffset.left,
    bottom: windowDim.height + windowOffset.top,
  };
};
...
Object.extend(Event, {
  findElementByAttr: function (event, tagName, attribute, value, flag) {
    ...
    while (
      !element[attribute] ||
      (flag
        ? element[attribute].indexOf(value) != 0
...
Richfaces.Calendar.parseDate = function (
  dateString,
  pattern,
  monthNames,
  monthNamesShort
  ...
  if (match != null) {
    ...
    else if (yy < 70) yy += 2000;
    else if (yy < 100) yy += 1900;
    var mm = parseInt(match[m], 10);
    if (isNaN(mm))
      mm = Richfaces.Calendar.getMonthByLabel(
        match[m],
        shortLabel ? monthNamesShort : monthNames
      );
    else if (--mm < 0 || mm > 11) return null;
    var dd = parseInt(match[d], 10);
    if (isNaN(dd) || dd < 1 || dd > daysInMonth(yy, mm)) return null;
...
Richfaces.Calendar.formatDate = function (
  date,
  pattern,
  monthNames,
  monthNamesShort
  ...
  var result = pattern.replace(
    /(\\\\|\\[yMdaHhm])|(y+|M+|d+|a|H{1,2}|h{1,2}|m{2})/g,
    function ($1, $2, $3) {
...
function weekNumber(year, month, mdifw, fdow) {
  ...
  weeknumber =
    o.weekNumber + Math.floor((d.getTime() - o.date.getTime()) / oneweek);
  return weeknumber;
}
...
Object.extend(Calendar.prototype, {
  initialize: function (id, locale, options, markups) {
    ...
    this.popupOffset = {
      dx: this.params.horizontalOffset,
      dy: this.params.verticalOffset,
    };
    ...
    this.secondWeekendDayNumber =
      this.params.firstWeekDay > 0 ? 7 - this.params.firstWeekDay : 0;
    ...
    var htmlTextHeader =
      '<table id="' +
      this.id +
      '" border="0" cellpadding="0" cellspacing="0" class="rich-calendar-exterior rich-calendar-popup ' +
      ...
      '" onclick="' +
      tempStr +
      'skipEventOnCollapse=true;"><tbody>';
    var colspan = this.params.showWeeksBar ? "8" : "7";
    var htmlHeaderOptional = this.params.optionalHeaderMarkup
      ? '<tr><td class="rich-calendar-header-optional" colspan="' +
        colspan +
        '" id="' +
        this.id +
        'HeaderOptional"></td></tr>'
      : "";
    var htmlFooterOptional = this.params.optionalFooterMarkup
      ? '<tr><td class="rich-calendar-footer-optional" colspan="' +
        colspan +
        '" id="' +
        this.id +
        'FooterOptional"></td></tr>'
      : "";
    var htmlControlsHeader = this.params.showHeader
      ? '<tr><td class="rich-calendar-header" colspan="' +
        colspan +
        '" id="' +
        this.id +
        'Header"></td></tr>'
      : "";
    var htmlControlsFooter = this.params.showFooter
      ? '<tr><td class="rich-calendar-footer" colspan="' +
        colspan +
        '" id="' +
        this.id +
        'Footer"></td></tr>'
    ...
    var htmlTextIFrame =
      '<iframe src="javascript:\'\'" frameborder="0" scrolling="no" id="' +
      this.IFRAME_ID +
      '" style="display:none; position: absolute; width: 1px; height: 1px; background-color:white;">' +
      "</iframe>";
    var styleClass;
    ...
    var context;
    var eventsStr =
      this.params.disabled || this.params.readonly
        ? ""
        : 'onclick="' +
          tempStr +
          'eventCellOnClick(event, this);" onmouseover="' +
          tempStr +
          'eventCellOnMouseOver(event, this);" onmouseout="' +
          tempStr +
          'eventCellOnMouseOut(event, this);"';
    if (this.params.showWeekDaysBar) {
      htmlTextWeekDayBar.push('<tr id="' + this.WEEKDAY_BAR_ID + '">');
      if (this.params.showWeeksBar)
        htmlTextWeekDayBar.push('<td class="rich-calendar-days"><br/></td>');
      var weekDayCounter = this.params.firstWeekDay;
      for (var i = 0; i < 7; i++) {
        context = {
          weekDayLabel: this.params.weekDayLabels[weekDayCounter],
          weekDayLabelShort: this.params.weekDayLabelsShort[weekDayCounter],
          weekDayNumber: weekDayCounter,
          isWeekend: this.isWeekend(i),
          elementId: this.WEEKDAY_ELEMENT_ID + i,
          component: this,
        };
        var weekDayHtml = this.evaluateMarkup(
          this.params.weekDayMarkup,
          context
        );
        if (weekDayCounter == 6) weekDayCounter = 0;
        else weekDayCounter++;
    ...
    for (k = 1; k < 7; k++) {
      ...
      if (this.params.showWeeksBar) {
        context = {
          weekNumber: k,
          elementId: this.WEEKNUMBER_ELEMENT_ID + k,
          component: this,
        };
        var weekNumberHtml = this.evaluateMarkup(
          this.params.weekNumberMarkup,
          context
        );
        htmlTextWeek.push(
      ...
      for (var i = 0; i < 7; i++) {
        ...
        htmlTextWeek.push(
          '<td class="' +
            styleClass +
            '" id="' +
            this.DATE_ELEMENT_ID +
            p +
            '" ' +
            eventsStr +
            ">" +
            (this.customDayListMarkup
              ? '<div class="rich-calendar-cell-div' +
                (this.params.dayCellClass
                  ? " " + this.params.dayCellClass
                  : "") +
                '"></div>'
  ...
  scrollEditorYear: function (value) {
    ...
    if (element) {
      var div;
      var year = (this.dateEditorStartYear =
        this.dateEditorStartYear + value * 10);
      for (var i = 0; i < 5; i++) {
  ...
  createEditor: function () {
    ...
    var htmlBegin =
      '<div id="' +
      this.EDITOR_SHADOW_ID +
      '" class="rich-calendar-editor-shadow" style="position:absolute; display:none;"></div><table border="0" cellpadding="0" cellspacing="0" id="' +
      this.EDITOR_ID +
      '" style="position:absolute; display:none;" onclick="$(\'' +
      this.id +
      '\').component.skipEventOnCollapse=true;"><tbody><tr><td class="rich-calendar-editor-container" align="center"><div style="position:relative; width:100%">';
    var htmlContent =
      '<div id="' +
      this.EDITOR_LAYOUT_SHADOW_ID +
  ...
  createTimeEditorLayout: function (editor) {
    Element.insert(this.EDITOR_LAYOUT_SHADOW_ID, {
      after: this.evaluateMarkup(
        this.calendarContext.timeEditorLayout,
    ...
    var tm = $(this.id + "TimeMinutes");
    if (this.timeType == 1) {
      sbjQuery(th).SpinButton({
        digits: this.timeHoursDigits,
        min: 0,
        max: 23,
      });
    } else {
    ...
    this.correctEditorButtons(
      editor,
      this.TIME_EDITOR_BUTTON_OK,
  ...
  correctEditorButtons: function (editor, buttonID1, buttonID2) {
    ...
    var width1 = Richfaces.Calendar.getOffsetDimensions(
      button1.firstChild
    ).width;
    var width2 = Richfaces.Calendar.getOffsetDimensions(
      button2.firstChild
    ).width;
    ...
    if (width1 > styleWidth || width2 > styleWidth) {
      button1.style.width = button2.style.width =
        (width1 > width2 ? width1 : width2) + "px";
    }
  },
    ...
    if (buttonType == 0) {
      ...
      var onclick =
        buttonType == 1
          ? "$('" +
            this.id +
            "').component.dateEditorSelectMonth(" +
            param +
            ");"
          : "$('" +
            this.id +
            "').component.dateEditorSelectYear(" +
            param +
            ");";
      return (
        '<div id="' +
        ...
        '" class="rich-calendar-editor-btn' +
        (className ? " " + className : "") +
        '" onmouseover="Element.addClassName(this, \'rich-calendar-editor-btn-over\');" onmouseout="Element.removeClassName(this,\'rich-calendar-editor-btn-over\');" onclick="' +
        onclick +
        '">' +
        value +
        "</div>"
  ...
  createDateEditorLayout: function (editor) {
    var htmlBegin =
      '<table id="' +
      this.DATE_EDITOR_LAYOUT_ID +
      '" class="rich-calendar-date-layout" border="0" cellpadding="0" cellspacing="0"><tbody><tr id="' +
      this.DATE_EDITOR_LAYOUT_ID +
      'TR">';
    ...
    var htmlContent =
      '<td align="center">' +
      this.createDECell(
        this.DATE_EDITOR_LAYOUT_ID + "M" + month,
        this.params.monthLabelsShort[month],
        1,
        month
      ) +
      "</td>" +
      '<td align="center" class="rich-calendar-date-layout-split">' +
      this.createDECell(
        this.DATE_EDITOR_LAYOUT_ID + "M" + (month + 6),
        this.params.monthLabelsShort[month + 6],
        1,
        month + 6
      ) +
      "</td>" +
    ...
    for (var i = 0; i < 5; i++) {
      htmlContent +=
        ...
        '<td align="center">' +
        this.createDECell(
          this.DATE_EDITOR_LAYOUT_ID + "Y" + i,
          year,
          2,
          i,
          i == 4 ? "rich-calendar-editor-btn-selected" : ""
        ) +
        "</td>" +
        '<td align="center">' +
        this.createDECell(
          this.DATE_EDITOR_LAYOUT_ID + "Y" + (i + 5),
          year + 5,
          2,
          i + 5
        ) +
    ...
    this.dateEditorMonthID =
      this.DATE_EDITOR_LAYOUT_ID + "M" + this.dateEditorMonth;
    htmlContent +=
      '</tr><tr><td colspan="2" class="rich-calendar-date-layout-ok">' +
      '<div id="' +
      this.DATE_EDITOR_BUTTON_OK +
      '" class="rich-calendar-time-btn" style="float:right;" onmousedown="Element.addClassName(this, \'rich-calendar-time-btn-press\');" onmouseout="Element.removeClassName(this, \'rich-calendar-time-btn-press\');" onmouseup="Element.removeClassName(this, \'rich-calendar-time-btn-press\');" onclick="$(\'' +
      this.id +
      "').component.hideDateEditor(true);\"><span>" +
      this.params.labels.ok +
      "</span></div>" +
      '</td><td colspan="2" class="rich-calendar-date-layout-cancel">' +
      '<div id="' +
      this.DATE_EDITOR_BUTTON_CANCEL +
      '" class="rich-calendar-time-btn" style="float:left;" onmousedown="Element.addClassName(this, \'rich-calendar-time-btn-press\');" onmouseout="Element.removeClassName(this, \'rich-calendar-time-btn-press\');" onmouseup="Element.removeClassName(this, \'rich-calendar-time-btn-press\');" onclick="$(\'' +
      this.id +
      "').component.hideDateEditor(false);\"><span>" +
      this.params.labels.cancel +
      "</span></div>" +
      "</td>";
    Element.insert(this.EDITOR_LAYOUT_SHADOW_ID, {
      after: htmlBegin + htmlContent + htmlEnd,
    });
    Element.addClassName(
      this.dateEditorMonthID,
      "rich-calendar-editor-btn-selected"
  ...
  createSpinnerTable: function (id) {
    return (
      ...
      '<div id="' +
      id +
      'BtnUp" class="rich-calendar-spinner-up"' +
      ...
      '<div id="' +
      id +
      'BtnDown" class="rich-calendar-spinner-down"' +
  ...
  setTimeProperties: function () {
    ...
    while ((r = re.exec(dateTimePattern)))
      if (!r[1]) pattern.push({ str: r[0], marker: r[2], idx: r.index });
    var datePattern = "";
    var timePattern = "";
    var digits, h, hh, m, a;
    var id = this.id;
    var getString = function (p) {
      return p.length == 0
        ? obj.marker
        : dateTimePattern.substring(
            pattern[i - 1].str.length + pattern[i - 1].idx,
    ...
    var calendar = this;
    this.timePatternHtml = timePattern.replace(
      /(\\\\|\\[yMdaHhm])|(H{1,2}|h{1,2}|m{2}|a)/g,
      function ($1, $2, $3) {
        if ($2) return $2.charAt(1);
        switch ($3) {
          case "a":
            return (
              "</td><td>" +
              calendar.createSpinnerTable(id + "TimeSign") +
              "</td><td>"
            );
          case "H":
          case "HH":
          ...
          case "hh":
            return (
              "</td><td>" +
              calendar.createSpinnerTable(id + "TimeHours") +
              "</td><td>"
            );
          case "mm":
            return (
              "</td><td>" +
              calendar.createSpinnerTable(id + "TimeMinutes") +
              "</td><td>"
            );
        }
      }
  ...
  doExpand: function (e) {
    ...
    if (this.invokeEvent("expand", element, e)) {
      ...
      if (this.params.showInput) {
        ...
        offsetBase = [
          offsetBase[0] < offsetBase1[0] ? offsetBase[0] : offsetBase1[0],
      ...
      var o = {
        ...
        height:
          offsetDimInput && offsetDimInput.height > offsetDimButton.height
            ? offsetDimInput.height
            : offsetDimButton.height,
      };
      Richfaces.Calendar.setElementPosition(
        element,
        o,
        this.params.jointPoint,
      ...
      this.scrollElements = Richfaces.setupScrollEventHandlers(
        element,
        this.eventOnScroll
      );
    }
  ...
  eventOnCollapse: function (e) {
    ...
    if (
      Event.element(e).id == this.POPUP_BUTTON_ID ||
      (!this.params.enableManualInput &&
        Event.element(e).id == this.INPUT_DATE_ID)
    )
      return true;
    if (Position.within($(this.id), Event.pointerX(e), Event.pointerY(e)))
      return true;
    this.doCollapse();
    return true;
  },
  ...
  getSelectedDateString: function (pattern) {
    ...
    return Richfaces.Calendar.formatDate(
      this.selectedDate,
      pattern,
      this.params.monthLabels,
  ...
  isWeekend: function (weekday) {
    return (
      weekday == this.firstWeekendDayNumber ||
      weekday == this.secondWeekendDayNumber
    );
  },
  ...
  setupTimeForDate: function (date) {
    if (
      this.selectedDate &&
      (!this.params.resetTimeOnDateSelect ||
  ...
  eventCellOnClick: function (e, obj) {
    var daydata =
      this.days[parseInt(obj.id.substr(this.DATE_ELEMENT_ID.length), 10)];
    if (daydata.enabled && daydata._month == 0) {
      ...
      if (
        this.selectDate(date, true, { event: e, element: obj }) &&
      ...
      if (this.params.boundaryDatesMode == "scroll")
        if (daydata._month == -1) this.prevMonth();
        else this.nextMonth();
        ...
        if (
          this.selectDate(date, false, { event: e, element: obj }) &&
          !this.params.showApplyButton
        ) {
          this.doCollapse();
        }
  ...
  eventCellOnMouseOver: function (e, obj) {
    var daydata =
      this.days[parseInt(obj.id.substr(this.DATE_ELEMENT_ID.length), 10)];
    if (
      this.invokeEvent("datemouseover", obj, e, daydata.date) &&
      daydata.enabled
    ) {
      if (
        daydata._month == 0 &&
        obj.id != this.selectedDateCellId &&
  ...
  eventCellOnMouseOut: function (e, obj) {
    var daydata =
      this.days[parseInt(obj.id.substr(this.DATE_ELEMENT_ID.length), 10)];
    if (
      this.invokeEvent("datemouseout", obj, e, daydata.date) &&
      daydata.enabled
    ) {
      if (
        daydata._month == 0 &&
        ...
        Element.removeClassName(obj, "rich-calendar-hover");
    }
  },
  ...
  indexData: function (daysData, isAjaxMode) {
    ...
    var idx =
      daysInMonthByDate(daysData.startDate) - daysData.startDate.getDate() + 1;
  ...
  render: function () {
    ...
    if (wd > 0)
      while (dayCounter <= previousMonthDays) {
        this.days.push({
          day: dayCounter,
          isWeekend: this.isWeekend(p),
          _month: month,
        });
        dayCounter++;
        p++;
    ...
    if (
      ...
      if (
        this.daysData.startDate.getFullYear() == currentYear &&
        this.daysData.startDate.getMonth() == currentMonth
      ) {
        var firstDay = (firstDay = this.daysData.days[idx].day
          ? this.daysData.days[idx].day
          : this.daysData.startDate.getDate());
        while (dayCounter < firstDay) {
          this.days.push({
            day: dayCounter,
            isWeekend: this.isWeekend(p % 7),
            _month: month,
          });
          dayCounter++;
          p++;
    ...
    if (this.params.showWeeksBar)
      wn = weekNumber(
        ...
        this.params.firstWeekDay
      );
    this.selectedDayElement = null;
    var weekflag = true;
    var e;
    var boundaryDatesModeFlag =
      this.params.boundaryDatesMode == "scroll" ||
      this.params.boundaryDatesMode == "select";
    this.todayCellId = this.clearEffect(this.todayCellId, this.highlightEffect);
    this.selectedDateCellId = this.clearEffect(
    ...
    for (var k = 1; k < 7; k++) {
      ...
      if (this.params.showWeeksBar) {
        if (
          ...
          (k == 5 || k == 6) &&
          (dataobj._month == 1 ||
            7 - (currentMonthDays - dataobj.day + 1) >=
              this.params.minDaysInFirstWeek)
        ) {
          wn = 1;
          weekflag = false;
      ...
      while (element) {
        ...
        if (dataobj.enabled != false)
          dataobj.enabled = this.params.isDayEnabled(dataobj);
        if (!dataobj.styleClass)
          dataobj.customStyleClass = this.params.dayStyleClass(dataobj);
        else {
        ...
        contentElement = this.customDayListMarkup
          ? element.firstChild
          : element;
        contentElement.innerHTML = this.evaluateMarkup(
  ...
  renderHF: function () {
    if (this.params.showHeader)
      this.renderMarkup(
        this.params.headerMarkup,
        this.id + "Header",
        this.calendarContext
      );
    if (this.params.showFooter)
      this.renderMarkup(
        this.params.footerMarkup,
        this.id + "Footer",
        this.calendarContext
      );
  ...
  renderHeaderOptional: function () {
    this.renderMarkup(
      this.params.optionalHeaderMarkup,
  ...
  renderFooterOptional: function () {
    this.renderMarkup(
      this.params.optionalFooterMarkup,
      this.id + "FooterOptional",
      this.calendarContext
    );
  },
  ...
  onUpdate: function () {
    var formattedDate = Richfaces.Calendar.formatDate(
      this.getCurrentDate(),
      "MM/yyyy"
    );
    $(this.id + "InputCurrentDate").value = formattedDate;
    if (this.submitFunction) this.submitFunction(formattedDate);
    else this.render();
  },
  nextMonth: function () {
  ...
  changeCurrentDateOffset: function (yearOffset, monthOffset) {
    var date = new Date(
      this.currentDate.getFullYear() + yearOffset,
  ...
  today: function (noUpdate, noHighlight) {
    ...
    if (
      nowyear != this.currentDate.getFullYear() ||
      nowmonth != this.currentDate.getMonth()
    ) {
      updateflag = true;
    ...
    if (updateflag) {
      ...
      if (this.isVisible && this.todayCellId && !noHighlight) {
        ...
        if (this.todayCellColor != "transparent") {
          this.highlightEffect = new Effect.Highlight($(this.todayCellId), {
            startcolor: this.todayCellColor,
            duration: 0.3,
    ...
    if (
      this.params.todayControlMode == "select" &&
      !this.params.disabled &&
      !this.params.readonly
    )
  ...
  selectToday: function () {
    if (this.todayCellId) {
      var daydata =
        this.days[
          parseInt(
            $(this.todayCellId).id.substr(this.DATE_ELEMENT_ID.length),
            10
          )
        ];
      var today = new Date();
  ...
  selectDate: function (date, noUpdate, eventData) {
    ...
    if (date) {
      if (typeof date == "string") {
        date = Richfaces.Calendar.parseDate(
          date,
          this.params.datePattern,
          this.params.monthLabels,
          this.params.monthLabelsShort
        );
      }
    ...
    if (
      ...
      (oldSelectedDate != null || newSelectedDate != null)
    ) {
      isDateChange = true;
      flag = this.invokeEvent(
        "dateselect",
        eventData.element,
        eventData.event,
        date
    ...
    if (flag) {
      if (newSelectedDate != null) {
        if (
          newSelectedDate.getMonth() == this.currentDate.getMonth() &&
          ...
          if (!oldSelectedDate || oldSelectedDate - this.selectedDate) {
            var e = $(
              this.DATE_ELEMENT_ID +
                (this.firstDateIndex + this.selectedDate.getDate() - 1)
            );
            this.clearEffect(
              this.selectedDateCellId,
              this.highlightEffect2,
      ...
      if (isDateChange) {
        ...
        if (!this.params.showApplyButton) {
          this.setInputField(
            this.selectedDate != null
              ? this.getSelectedDateString(this.params.datePattern)
              : "",
            eventData.event
          );
        }
    ...
    if (
      ...
      if (this.isVisible && this.selectedDateCellId) {
        ...
        if (this.selectedDateCellColor != "transparent") {
          this.highlightEffect2 = new Effect.Highlight(
            $(this.selectedDateCellId),
            {
              startcolor: this.selectedDateCellColor,
              duration: 0.3,
              transition: Effect.Transitions.sinoidal,
              afterFinish: this.onHighlightFinish,
            }
  ...
  hideTimeEditor: function (updateTime) {
    ...
    if (updateTime && this.selectedDate) {
      ...
      if (
        ...
        this.invokeEvent("timeselect", null, null, date)
      ) {
  ...
  var attr = Object.extend(
    {
      onclick:
        (functionName
          ? "Richfaces.getComponent('calendar',this)." +
            functionName +
            "(" +
            (paramsStr ? paramsStr : "") +
            ");"
...
CalendarView.getSelectedDateControl = function (calendar) {
  ...
  var text = Richfaces.Calendar.formatDate(
    calendar.selectedDate,
    calendar.timeType ? calendar.datePattern : calendar.params.datePattern,
    calendar.params.monthLabels,
    calendar.params.monthLabelsShort
  );
  var onclick =
    "Richfaces.getComponent('calendar',this).showSelectedDate(); return true;";
  var markup = calendar.params.disabled
    ? new E("div", { class: "rich-calendar-tool-btn-disabled" }, [new ET(text)])
    : new E("div", { class: "rich-calendar-tool-btn", onclick: onclick }, [
        new ET(text),
      ]);
  return markup;
};
CalendarView.getTimeControl = function (calendar) {
  ...
  var text = Richfaces.Calendar.formatDate(
    calendar.selectedDate,
    calendar.timePattern,
    calendar.params.monthLabels,
    calendar.params.monthLabelsShort
  );
  var onmouseover =
    "Element.removeClassName(this, 'rich-calendar-tool-btn-press');";
  var onmouseout =
    "Element.addClassName(this, 'rich-calendar-tool-btn-press');";
  var onclick =
    "Richfaces.getComponent('calendar',this).showTimeEditor();return true;";
  var markup =
    calendar.params.disabled || calendar.params.readonly
      ...
      : new E(
          ...
          {
            class:
              "rich-calendar-tool-btn rich-calendar-tool-btn-hover rich-calendar-tool-btn-press",
            onclick: onclick,
...
CalendarView.toolButtonAttributes = {
  className: "rich-calendar-tool-btn",
  onmouseover:
    "this.className='rich-calendar-tool-btn rich-calendar-tool-btn-hover'",
  onmouseout: "this.className='rich-calendar-tool-btn'",
  onmousedown:
    "this.className='rich-calendar-tool-btn rich-calendar-tool-btn-hover rich-calendar-tool-btn-press'",
  onmouseup:
    "this.className='rich-calendar-tool-btn rich-calendar-tool-btn-hover'",
};
CalendarView.nextYearControl = function (context) {
  return !context.calendar.params.disabled
    ? CalendarView.getControl(
        ">>",
        CalendarView.toolButtonAttributes,
        "nextYear"
      )
...
CalendarView.currentMonthControl = function (context) {
  var text = Richfaces.Calendar.formatDate(
    context.calendar.getCurrentDate(),
    "MMMM, yyyy",
    context.monthLabels,
    context.monthLabelsShort
  );
  var markup = context.calendar.params.disabled
    ? new E("div", { className: "rich-calendar-tool-btn-disabled" }, [
        new T(text),
      ])
    : CalendarView.getControl(
        text,
...
CalendarView.applyControl = function (context) {
  return !context.calendar.params.disabled &&
    !context.calendar.params.readonly &&
    context.calendar.params.showApplyButton
    ? CalendarView.getControl(
        context.controlLabels.apply,
        CalendarView.toolButtonAttributes,
        "close",
        "true"
      )
...
CalendarView.cleanControl = function (context) {
  return !context.calendar.params.disabled &&
    ...
    ? CalendarView.getControl(
        context.controlLabels.clean,
        CalendarView.toolButtonAttributes,
        "resetSelectedDate"
      )
    : "";
};
CalendarView.selectedDateControl = function (context) {
  return CalendarView.getSelectedDateControl(context.calendar);
};
CalendarView.timeControl = function (context) {
  return CalendarView.getTimeControl(context.calendar);
};
CalendarView.timeEditorFields = function (context) {
        ...
        new E("tr", {}, [
          new E("td", { class: "rich-calendar-tool" }, [
            new ET(function (context) {
              return Richfaces.evalMacro("previousYearControl", context);
            }),
          ]),
          new E("td", { class: "rich-calendar-tool" }, [
            new ET(function (context) {
              return Richfaces.evalMacro("previousMonthControl", context);
            }),
          ]),
          new E("td", { class: "rich-calendar-month" }, [
            new ET(function (context) {
              return Richfaces.evalMacro("currentMonthControl", context);
            }),
          ]),
          new E("td", { class: "rich-calendar-tool" }, [
            new ET(function (context) {
              return Richfaces.evalMacro("nextMonthControl", context);
            }),
          ]),
          new E("td", { class: "rich-calendar-tool" }, [
            new ET(function (context) {
              return Richfaces.evalMacro("nextYearControl", context);
            }),
          ]),
          new E(
            "td",
            {
              class: "rich-calendar-tool rich-calendar-tool-close",
            ...
            [
              new ET(function (context) {
                return Richfaces.evalMacro("closeControl", context);
              }),
            ]
          ),
        ]),
      ...
      new E("tbody", {}, [
        new E("tr", {}, [
          new E(
            "td",
            {
              class: "rich-calendar-toolfooter",
              style: function (context) {
                return this.isEmpty ? "display:none;" : "";
              },
            },
            [
              new ET(function (context) {
                return Richfaces.evalMacro("selectedDateControl", context);
              }),
            ]
          ),
          new E(
            ...
            [
              new ET(function (context) {
                return Richfaces.evalMacro("cleanControl", context);
              }),
            ]
          ),
          new E(
            ...
            [
              new ET(function (context) {
                return Richfaces.evalMacro("timeControl", context);
              }),
            ]
          ),
          new E(
          ...
          new E(
            "td",
            {
              ...
              style: function (context) {
                return (
                  (this.isEmpty ? "display:none;" : "") +
                  (context.calendar.params.disabled ||
                  context.calendar.params.readonly ||
                  !context.calendar.params.showApplyButton
            ...
            [
              new ET(function (context) {
                return Richfaces.evalMacro("todayControl", context);
              }),
            ]
          ),
          new E(
            ...
            [
              new ET(function (context) {
                return Richfaces.evalMacro("applyControl", context);
              }),
            ]
          ),
        ]),
...
CalendarView.timeEditorLayout = [
  new E(
    "table",
    {
      ...
      cellpadding: "0",
      cellspacing: "0",
      class: "rich-calendar-time-layout",
    },
    [
      new E("tbody", {}, [
        new E("tr", {}, [
          new E(
            "td",
            {
              class: "rich-calendar-time-layout-fields",
              colspan: "2",
              align: "center",
            },
            [
              new ET(function (context) {
                return Richfaces.evalMacro("timeEditorFields", context);
              }),
            ]
          ),
        ]),
              ...
              "div",
              {
                ...
                onclick: function (context) {
                  return (
                    "$('" +
                    context.calendar.id +
                    "').component.hideTimeEditor(false)"
                  );
                },
              },
              [
