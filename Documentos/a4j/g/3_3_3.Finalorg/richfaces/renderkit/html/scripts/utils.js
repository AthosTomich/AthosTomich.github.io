...
Richfaces.getNSAttribute = function (name, element) {
  if (element.getAttributeNS) {
    var attr = element.getAttributeNS(
...
Richfaces.Position.setElementPosition = function (
  element,
  baseElement,
  jointPoint,
  direction,
  ...
  if (direction && (match = direction.toLowerCase().match(re)) != null) {
    ...
    if (match[2] == "left") {
      ox -= elementDim.width + offset.x;
    } else ox += offset.x;
    if (match[1] == "top") {
      oy -= elementDim.height + offset.y;
    } else oy += offset.y;
  } else {
    var theBest = { square: 0 };
    var rect = {
      right: baseOffset[0] + baseElementDim.width,
      top: baseOffset[1] + baseElementDim.height,
    };
...
Richfaces.Position.checkCollision = function (
  elementRect,
  windowRect,
  windowOffset
) {
